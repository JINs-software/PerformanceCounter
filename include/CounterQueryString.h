...
#define dfQUERY_PRECESS_HANDLE_COUNT		L"\\Process(@param)\\Handle Count"			// 프로세스 핸들 수 
#define dfQUERY_PROCESS_THREAD_COUNT		L"\\Process(@param)\\Thread Count"			// 프로세스 스레드 수
#define dfQUERY_PROCESS_VMEMORY_USAGE		L"\\Process(@param)\\Virtual Bytes"			// 프로세스 가상메모리 사용(가상메모리 테이블의 주소 용량으로 실제 사용메모리와는 무관)
#define dfQUERY_PROCESS_USER_VMEMORY_USAGE	L"\\Process(@param)\\Private Bytes"			// 프로세스 유저할당 메모리(실제 유저 메모리 사용량, 프로세스 공용메모리/커널메모리 제외)
#define dfQUERY_PROCESS_WORKING_SET			L"\\Process(@param)\\Working Set"			// 프로세스 작업 메모리(물리 메모리에 사용되는 크기일 뿐 할당 용량은 아닐 수 있음)
#define dfQUERY_PROCESS_NON_PAGED_MEMORY	L"\\Process(@param)\\Pool Nonpaged Bytes"	// 프로세스 논페이지 메모리

#define dfQUERY_THREAD_CPU_USAGE			L"\\Thread(@param)\\% Processor Time"
